...
// healthy. So low occupancy grades RED and a near-full centre GREEN.
// Discrete RAG bands (not a continuous blend) so every bar is cleanly
...
export function occupancyBand(pct: number): "green" | "amber" | "red" {
  if (pct >= OCCUPANCY_GREEN_MIN) return "green";
  if (pct >= OCCUPANCY_AMBER_MIN) return "amber";
  return "red";
}

export function occupancyColor(pct: number): string {
  return ragAccent[occupancyBand(pct)];
}
